(function (exp, $) {
    exp.KeyGenerator = function () {
        ...
        var getTotalLength = function (words) {
            return words.join("").length;
        };

        var removeIgnoredWords = function (words) {
            return $.grep(words, function (word, index) {
                return $.inArray(word, IGNORED_WORDS) === -1;
            });
        };

        var createAcronym = function (words) {
            var result = "";
        ...
        return {
            generateKey: function (name, options) {
                ...
                var maxKeyLength = (typeof options.maxKeyLength === 'number') ? options.maxKeyLength : Infinity;
                var charBlacklistRegex = (typeof options.charBlacklistRegex !== 'undefined'? options.charBlacklistRegex : /[^a-zA-Z0-9]/g);
                ...
                $.each(name.split(/\s+/), function (i, word) {
                    if (word) {
                        ...
                        // uppercase the word (NOTE: JavaScript attempts to convert characters like À? in to SS)
                ...
                if (words.length === 0) {
                    ...
                    var acronym = createAcronym(words);
    ...
    $.fn.generateFrom = function ($nameElem, opts) {
        var defaultOptions = {
            desiredKeyLength: 4,
            maxKeyLength: 10,
            maxNameLength: 30,
        ...
        var $keyElement = $(this).first();
        var $nameElement = $nameElem.first();
        ...
        (function () {
            ...
            var onNameTimeout = function () {
                setKeyEdited($keyElement.val());
                autofillKeyIfNeeded();
            };

            var bindNameHook = function (e) {
                bindHook(e, onNameTimeout);
            };

            var bindHook = function (e, func) {
                var el = $(e.target);
                var hook;
                hook = function () {
                    unbindHook(e);
            ...
            var autofillKeyIfNeeded = function () {
                if (shouldUpdateKey()) {
