/**
 * @module confluence/avataror
 */
define('confluence/avataror', [
    'jquery'
], function(
    $
) {
    ...
    return function() {
        $('.avataror').each(function() {
            ...
            avataror.img.load(function() {
                avataror.image = $('<div></div>').css({
                    background: 'url(\'' + imgsrc + '\') no-repeat',
                    clear: 'left',
                    position: 'relative'
                });
                avataror.marker = $('<div></div>').css({ cursor: 'move', position: 'relative' });
                avataror.dash = $('<div></div>');
                avataror.shadow = $('<div></div>');
                avataror.dash.add(avataror.shadow).css({
                    cursor: 'move',
                    opacity: 0.5,
                    left: 0,
                    top: 0,
                ...
                avataror.preview.css({
                    margin: '0 0 10px ' + Math.round(avataror.imgwidth / 2 - avataror.previewSize / 2) + 'px',
                    position: 'static'
                });
                $('p', $this).remove();
            });
...
require('confluence/module-exporter').safeRequire('confluence/avataror', function(Avataror) {
    'use strict';
