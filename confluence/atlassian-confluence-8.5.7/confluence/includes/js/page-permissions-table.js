 ...
 * @module confluence/page-permissions-table
 */
define('confluence/page-permissions-table', [
    'jquery',
    'confluence/meta',
    'ajs',
    ...
    function PagePermissionsTable($table) {
        var t = this;
        ...
        this.refresh = function() {
            $('#page-permissions-table-div').show();
            $('#page-inherited-permissions-table-div').show();
            ...
            AJSOverrides.setVisible('#page-permissions-no-views', !hasViewPermissions);
            AJSOverrides.setVisible('#page-permissions-no-edits', !hasEditPermissions);
        ...
        this.makePermissionMap = function(getDisplayNames) {
            ...
            $table.find('tr.view-permission-row, tr.edit-permission-row').each(function() {
                ...
                if (entityType === 'user') {
                    ...
                    if (getDisplayNames) {
                        entityName = row.find('.permission-entity-display-name').text();
                    } else {
                        entityName = row.attr('data-userkey');
                    }
                } else {
                    entityName = row.find('.permission-entity-name').text();
                }
        ...
        this.makePermissionMapForCheckboxes = function(forSummary) {
            ...
            $table.find('tr.view-permission-row').each(function() {
                var row = $(this);
                var viewChecked = row.find('.view-permission-cell input').prop('checked');
                var editChecked = row.find('.edit-permission-cell input').prop('checked');
        ...
        var setupEntity = function(row, entity) {
            ...
            if (entity.type === 'group') {
                nameColumn.find('img').attr('src', AJS.contextPath() + '/images/icons/avatar_group_32.png');
                nameColumn.find('.permission-entity-name-wrap').hide();
            }

            if (entity.type === 'user') {
                nameColumn.find('img').attr('src', entity.avatarUrl);
                nameColumn.find('span.entity-container')
                    .addClass('content-hover user-hover-trigger')
                    .attr('data-username', entity.name);
                row.attr('data-userkey', entity.userKey);
            }
        };
        ...
        this.addRow = function(entry, permissionType) {
            ...
            var newRowElement = $(Templates.PagePermissions.permissionRow());
            ...
            if (entry.inherited) {
                ...
                if (!inheritedTable.length) {
                    ...
                    var newTableDiv = $(Templates.PagePermissions.inheritedPermissionsTable());
                    ...
                    var href = AJS.contextPath() + '/pages/viewpage.action?pageId=' + entry.owningId;
                    ...
                    var title = editorPageTitle.length ? editorPageTitle.val() : Meta.get('page-title');
                ...
                $('#page-inherited-permissions-table-desc').show();
        ...
        this.resetInherited = function() {
            $('#page-inherited-permissions-table-desc').hide();
        ...
        this.showErrorMessage = function(errorMessage) {
            $('#page-permissions-table-div').hide();
            $('#page-inherited-permissions-table-div').hide();

            $('#page-permissions-tables-error').remove();

            AJS.messages.warning('#page-permissions-tables', {
                title: errorMessage,
                id: 'page-permissions-tables-error',
                insert: 'prepend'
            });
        };
    ...
    return PagePermissionsTable;
});

require('confluence/module-exporter').exportModuleAsGlobal('confluence/page-permissions-table', 'AJS.PagePermissions.Table');
