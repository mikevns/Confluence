/**
 * @module confluence/page-move-dialog-browse
 */
define('confluence/page-move-dialog-browse', [
    'jquery',
    'ajs',
     ...
     */
    return function(controls, newSpaceKey, newSpaceName, newParentPage, originalParent, pageTitle) {
        var contextPath = $('#confluence-context-path').attr('content');
        var container = this;

        var treeContainer = $('.tree', container);
        treeContainer.addClass('loading').html(Templates.MovePage.panelLoading());
        ...
        var clickHandler = function(e) {
            ...
            if (!isEditorAjaxSaveEnabled) {
                ...
                // If user clicked on page, we can pass the page id to update "parentPageId" hidden field value
                if (newParentPage.length > 0) {
                    controls.select(newSpaceKey, newSpaceName, newParentPage, $(this).parent().attr('id').split('-')[1]);
                } else {
                    controls.select(newSpaceKey, newSpaceName, newParentPage, '');
                }
            }
        ...
        var initialiseSpacePicker = function() {
            ...
            $('#tree-root-div').html(Templates.MovePage.browsePanelSpace({ linkText: newSpaceName }))
...
require('confluence/module-exporter').exportModuleAsGlobal('confluence/page-move-dialog-browse', 'jQuery.fn.movePageBrowse');
