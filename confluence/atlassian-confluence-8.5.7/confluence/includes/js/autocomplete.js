/**
 * @module confluence/autocomplete
 */
define('confluence/autocomplete', [
    'jquery'
], function(
    $
) {
    ...
    // This function is copied from AUI 7 during AUI 8 upgrade
    ...
    $.fn.autocomplete = function(url, minlength, callback) {
            ...
            if (currentTextfieldValue.length >= minlength && currentTextfieldValue != input[0].lastQuery && currentTextfieldValue != input[0].lastSelectedValue) {
                ...
                + '</li>';
                    ...
                    $('li', ol).click(function(e) {
                        e.stopPropagation();
                        var temp = $('i.fullDetails', this).html();
                        select(temp);
                    }).hover(function() {
        ...
        function select(value) {
            ...
            if (value) {
                ...
                input.val(value);
                var callbackData = {
                    input: input, originalValue: originalValue, value: value, fullName: $('.focused i.fullName').text(), username: $('.focused i.username').text()
                };
...
require('confluence/module-exporter').safeRequire('confluence/autocomplete');
